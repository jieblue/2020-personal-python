...
class Data:
    ...
    def querybyu(self, user:str, event: str):
        if not user in self.localu.keys():
            return 0
        return self.localu[user][event]

    def querybyr(self, repo: str, event: str):
        if not self.localr.get(repo, 0):
            return 0
        return self.localr[repo][event]
    def querybyur(self,user:str,repo:str,event: str):
...
if __name__ == '__main__':
    # data=Data("0",1)
    # print(data.querybyu("jieblue","PushEvent"))
    # print(data.querybyr("zzz","PushEvent"))
    #print(data.querybyur("izuzero","izuzero/xe-module-ajaxboard","PushEvent"))
    run=Run()